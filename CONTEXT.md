...
- **Стаффинг** — `GET /staffing/staffers`, `POST /staffing/staffers`, `GET/PATCH/DELETE /staffing/staffers/{id}` (в теле опционально `staffing_budget_id` — привязка к бюджету стаффинга); contractors аналогично + documents (multipart upload/download); `GET /staffing/expenses?year=&project_id=`, `PUT /staffing/expenses/{project_id}/{year}/{month}`, `GET /staffing/expenses/summary?year=`; invoices upload/download/delete; budgets CRUD + `PUT .../month-plan`; **`GET /staffing/budgets/{id}/month-detail?year=`** — помесячный план/факт по привязанным стафферам. Факт в `GET /staffing/budgets/{id}` считается из `StafferMonthExpense` привязанных стафферов, не из суммы `StaffingExpense` по проектам. Файлы хранятся в `staffing_uploads/` (Docker volume). Лимит файла: 50 МБ.
