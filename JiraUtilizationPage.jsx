    ...
    <div>
      ...
      <div className="card" style={{ padding: '16px 20px', marginBottom: 20 }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-end', flexWrap: 'wrap', marginBottom: 14 }}>
          {/* Month selector → norm from settings */}
          <div className="form-group" style={{ margin: 0 }}>
            <label className="label">Месяц (норма часов)</label>
            <select
              className="select"
              value={selectedMonth}
          ...
          <div className="form-group" style={{ margin: 0, minWidth: 130 }}>
            <label className="label" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
              ...
              {isManualOverride && (
                <button
                  ...
                  style={{ padding: '0 4px', fontSize: 11, height: 18, lineHeight: 1 }}
                  onClick={resetNorm}
            ...
            <input
              ...
              style={{ width: '100%' }}
            />
            <div style={{ fontSize: 11, color: isManualOverride ? 'var(--accent)' : 'var(--text-3)', marginTop: 3 }}>
              {isManualOverride
                ? 'Задано вручную'
                : normHoursConfigured
                  ? `Из настроек (${MONTH_NAMES[selectedMonth - 1]})`
                  : 'Умолчание (40 ч)'}
            </div>
          </div>

          {!normHoursConfigured && !isManualOverride && (
            <div className="alert alert-warning" style={{ margin: 0, flex: 1, fontSize: 12 }}>
              Рабочие часы для {year} не настроены в{' '}
              <a href="/settings" style={{ color: 'var(--accent)', fontWeight: 600 }}>Настройках</a>
              . Используется 40 ч или задайте норму вручную.
            </div>
          )}

          {/* Drop zone */}
          <div style={{ flex: 1, minWidth: 240 }}>
            <label className="label">Выгрузка из Jira (.xlsx)</label>
            <div
